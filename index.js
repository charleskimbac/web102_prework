...
function addGamesToPage(games) {
    ...
    games.forEach((game) => {
        ...
        gameElement.innerHTML = `
            <img src=${game.img} style="max-width: 100%; height: auto;"> 
...
const totalGames = GAMES_JSON.reduce(acc => ++acc, 0);
...
const displayStr = `A total of $${GAMES_JSON.reduce((acc, game) => acc + game.pledged, 0).toLocaleString('en-US')} has been raised for ${GAMES_JSON.reduce((acc) => ++acc, 0)} games. Currently, ${numUnfundedGames} games remain unfunded. We need your help to fund these amazing games!`;
...
const [firstGameJson, secondGameJson] = sortedGames;

// create a new element to hold the name of the top pledge game, then append it to the correct element
const firstGameElement = document.createElement("p");
firstGameElement.innerHTML = firstGameJson.name;
firstGameContainer.append(firstGameElement);

// do the same for the runner up item
const secondGameElement = document.createElement("p");
secondGameElement.innerHTML = secondGameJson.name;
